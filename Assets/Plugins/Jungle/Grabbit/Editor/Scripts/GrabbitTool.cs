...
namespace Grabbit
{
    ...
    public class GrabbitTool : EditorTool
    {
        ...
        private GUIStyle logoLabelStyle;
        ...
        private Texture2D Logo
        {
            get
            {
                ...
                return settings ? settings.GrabbitLogo : Texture2D.whiteTexture;
            }
        }
        ...
        private Texture2D SceneLogo
        {
            get
            {
                ...
                return settings ? settings.GrabbitSceneLogo : Texture2D.whiteTexture;
            }
        }

        // Cached once (built lazily inside OnGUI, where GUI.skin is valid) so we don't allocate a new
        // GUIStyle every scene repaint.
        private GUIStyle LogoLabelStyle => logoLabelStyle ??= new GUIStyle(GUI.skin.label)
        {
            alignment = TextAnchor.MiddleCenter, fixedWidth = 30, fixedHeight = 28, richText = true
        };
...
/*                Debug.LogWarning(
        ...
        private void PreparePhysicsActivation()
        {
            ...
            var bodies = FindObjectsByType<Rigidbody>();
            ...
            {
#if UNITY_6000_4_OR_NEWER
                rens = FindObjectsByType<MeshRenderer>();
#elif UNITY_2023_2_OR_NEWER
        ...
        private void ResetPhysicsSettings()
        {
            ...
            else
            {
                ...
                // Also restore Unity's default solver iteration counts. Grabbit lowers these on activation
                // (settings.solverIterations/velocityIterations, default 1/1); this branch previously reset
                // gravity + sim mode but forgot the iterations, so the editor kept running PhysX at 1
                // iteration after every session until a domain reload.
        ...
        private void HandleInput()
        {
            ...
            // Read Shift from the event modifier flag, not KeyCode.LeftShift KeyDown/KeyUp: on macOS the
            // Editor does not deliver keyCode events for modifier keys ("None of the modifier keys work
            // with Event.current.keyCode on OS X"), so the old checks never fired and the hold-Shift-to-run
            // Fall/Point modes appeared completely broken on Mac.
            bool shiftHeld = Event.current.shift;

            if (settings.IsDynamicPauseMode)
            {
                // Hold Shift to run (fall / point); release to pause.
                bool wantPaused = !shiftHeld;
                if (settings.Paused != wantPaused)
                {
                    settings.Paused = wantPaused;
            ...
            {
                // Tap Shift to toggle pause (rising edge of the modifier flag).
        ...
        private void ConfigureLimitationZoneGUI(SceneView obj)
        {
            ...
            GUILayout.Label(settings.GrabbitLogoWhiter, LogoLabelStyle);
        ...
        private void GravityInput()
        {
            ...
            // camera.pixelWidth/Height are backing-store pixels but GUIPointToWorldRay expects logical GUI
            // points. On a Retina/HiDPI display (pixelsPerPoint > 1) the raw value placed the gravity gizmo
            // off-screen so it couldn't be re-aimed. Convert pixels -> points (no-op at 1x).
            var coord = new Vector2(SceneView.lastActiveSceneView.camera.pixelWidth,
                SceneView.lastActiveSceneView.camera.pixelHeight) / EditorGUIUtility.pixelsPerPoint;
        ...
        private void UpdatePhysics()
        {
            ...
            // UpdatePhysics runs on EditorApplication.update, which fires even with no Scene View open;
            // lastActiveSceneView can be null there, so guard the repaint.
        ...
        private void RotationModeLogic()
        {
            ...
            if (isToolSelected)
            {
                foreach (var handler in selectionHandlers)
                {
                    ...
                    SetBodyVelocityLimits(handler);
                    body.LinearDamping(body.LinearDamping() + settings.ExtraDragForRotationMode);
                }
            }
            else
            {
        ...
        private void RaycastMouse()
        {
            ...
            if (IsPrefabMode)
            {
                // Capture the actual hit count — PhysicsScene.Raycast returns it. Using rayHits.Length
                // (always the buffer size) made the loop process empty slots whose zeroed hit.point
                // pulled Point-mode placement toward the world origin inside a Prefab Stage.
                length = prefabModePhysicsScene.Raycast(mouseRay.origin, mouseRay.direction, rayHits);
            }
            else
            {
