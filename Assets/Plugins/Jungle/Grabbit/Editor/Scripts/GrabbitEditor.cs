...
namespace Grabbit
{
    public class GrabbitEditor : EditorWindow
    {
        ...
        public static GrabbitSettings GetOrFetchSettings(bool noWarning = false)
        {
            if (!CurrentSettings)
            {
                var ids = AssetDatabase.FindAssets("t:GrabbitSettings");
        ...
        private static void StartGrabbitOnKeyDown(GrabbitMode mode = GrabbitMode.PLACE)
        {
            if (!instance)
            {
                ...
                // Check if we should only start when focused on Scene View
                if (settings.OnlyStartOnSceneViewFocus && EditorWindow.focusedWindow && 
                    !(EditorWindow.focusedWindow is SceneView))
                {
                    return;
                }
            }
            else if (EditorApplication.isPlayingOrWillChangePlaymode && !CurrentSettings.UseHotKeyInPlayMode)
            {
                return;
            }
            else if (CurrentSettings.OnlyStartOnSceneViewFocus && EditorWindow.focusedWindow && 
                     !(EditorWindow.focusedWindow is SceneView))
            {
                return;
            }
        ...
        private static void StartGrabbit(GrabbitMode mode = GrabbitMode.PLACE, bool ignoreChangeOfTool = false)
        {
            ...
            if (!ignoreChangeOfTool)
            {
                ...
                {
                    CurrentSettings.CurrentMode = GrabbitMode.PLACE;
                }
        ...
        private static void FindOrCreateTool()
        {
            ...
            var grab = FindAnyObjectByType<GrabbitTool>();
#else
        ...
        {
            if (CurrentSettings.IsCurrentModeDynamicPause)
            {
                CurrentSettings.Paused = true;
                CurrentSettings.IsDynamicPauseMode = true;
            }
            else
            {
                CurrentSettings.Paused = false;
                CurrentSettings.IsDynamicPauseMode = false;
            }
        }
        ...
        private bool DisplayPointParams()
        {
            ...
            GUIHelper.DisplayBoolOption("Control Rotation", ref CurrentSettings.ControlRotation, false,
        ...
        private bool DisplayPreferenceParams()
        {
            ...
            modif = modif || GUIHelper.DisplayBoolOption(
                "Only Start When Focusing Scene View",
                ref CurrentSettings.OnlyStartOnSceneViewFocus, false,
                "Only allows Grabbit to start when a Scene View window is focused");
