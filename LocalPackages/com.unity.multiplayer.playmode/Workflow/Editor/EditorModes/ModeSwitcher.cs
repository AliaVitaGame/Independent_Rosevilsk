    ...
    static class ModeSwitcher
    {
        ...
        public static ContainerWindowProxy CurrentWindow
        {
            get
            {
                ...
                {
                    var value = SessionState.GetString(k_CurrentWindowIdKey, "0");
                    return ContainerWindowProxy.FromEntityId(EntityId.FromULong(ulong.Parse(value)));
                }
            ...
            set
            {
                if (value == null)
                {
                    SessionState.EraseString(k_CurrentWindowIdKey);
                    SessionState.EraseBool(k_CurrentWindowSetKey);
                }
                else
                {
                    SessionState.SetString(k_CurrentWindowIdKey, EntityId.ToULong(value.GetEntityId()).ToString());
