...
namespace Unity.Multiplayer.PlayMode.Scenarios.Editor.GraphsFoundation
{
    ...
    internal class Instance : ISerializationCallbackReceiver
    {
        ...
        [SerializeField] private List<NodeStatus> m_CurrentStatus = new List<NodeStatus>();
        private ExecutionStage m_CurrentStage = ExecutionStage.None;

        internal string Name => m_Name;
        internal bool Drifted { get => m_Drifted; set => m_Drifted = value; }
        ...
        {
            var state = ComputeInstanceState(m_CurrentStatus.Select(node => node.State).ToList());
            return state is ExecutionState.Running or ExecutionState.Active;
        }
